...
def PREPROCESS(lines):
    ...
    CA = np.zeros((data_n, 40,1), dtype=float)
...
if __name__ == '__main__':
        ...
        SEQ_CA = np.concatenate((SEQ,CA),axis = 2)
        #print(SEQ_CA[0])
        
        trainsize = int(0.7* len(data))
        train_x = SEQ_CA[0:trainsize,:]
        ...
        out = Dense(units=1,  activation="linear")(dropout_4)
        ...
        model.fit(train_x, train_y, batch_size=64, epochs=500, shuffle=True, validation_data=( val_x, val_y), callbacks=[checkpointer,earlystopper])
