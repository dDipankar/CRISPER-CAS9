...
def features_selected(data_file):
	...
	X,Y = data_file[0:44162,0:691], data_file[0:44162,691]
	#print(data_file.shape)
	regr = linear_model.ElasticNet(random_state=0, alpha=0.01, l1_ratio =1)
	regr.fit(X, Y)
	coef = regr.coef_
...
def train_gbr(data_file, features):
	...
	X_test = X[33000:,:]
	y_test = y[33000:]
	regr=GradientBoostingRegressor(random_state=0, n_estimators=750 ,learning_rate = 0.1)
	regr.fit(X_tr, y_tr)
	y_pred = regr.predict(X_test)
	#print(y_pred[0:40])
...
def train_ffnn(data_file, features):
	...
	#X_tr = X[0:31000,features]
	#y_tr = y[0:31000]
	#X_val = X[31000:33000,features]
	#y_val = y[31000:33000]
	#X_test = X[33000:,features]
	#y_test = y[33000:]
	...
	y_test = y[33000:]
	model = Sequential()
	model.add(Dense(units=256, input_dim=425, activation="relu",kernel_initializer='glorot_uniform')) 
...
if __name__ == '__main__':
	data_file = pd.read_csv('features_ecoli_NAR.csv', header=None, skiprows=0)
